...
impl<Fp: PrimeField> MPTPath<Fp> {
    ...
    /// the immediate value in leaf node calculation
    pub fn key_immediate(&self) -> Fp {
        self.hash_traces[0].2
    }
    ...
    pub fn create(
        ...
        let (mut hashes, mut hash_types, mut hash_traces) = 
            if let Some(fp) = leaf {      
